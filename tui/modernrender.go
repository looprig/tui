...
// queuedTag is the dim marker word prefixed to every queued-affordance row so a
// stacked, not-yet-running user message reads UNAMBIGUOUSLY as queued — distinct from
// both the bold committed user row it will promote to and the live assistant tail it
// sits between. The queue is placed inline in the live tail, so it spells out the state
// rather than relying on faintness + position alone.
const queuedTag = "queued"

// renderQueued renders the focused loop's pending queued-input affordances as dim,
// EXPLICITLY-tagged rows for the viewport: each message's first line of text, prefixed
// with the gray "▌ " accent bar and a faint "queued " tag, wrapped to width and rendered
// in styles.QueuedStyle. It is a compact one-line-per-message preview of a throwaway
// affordance, never committed to scrollback. Empty messages yields "" so renderFocused
// appends no queued rows.
func renderQueued(messages [][]content.Block, width int) string {
	...
	for _, blocks := range messages {
		text := queuedTag + "  " + firstLine(renderInlineBlocks(blocks))
