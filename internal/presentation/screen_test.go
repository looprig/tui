...
func TestModernCtrlTExpandsThinking(t *testing.T) {
	...
	if !m.collapse.GloballyCollapsed() {
...
func TestModernCompletionTrayMouseMotionSelectsRow(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The session tray adds its four-row top-spacer/title/count/separator header
			// before the two-row records and their spacer, so the SECOND record starts
			// seven rows down.
...
// take over, exactly as Screen does (the parity gate for a restored session showing blank).
